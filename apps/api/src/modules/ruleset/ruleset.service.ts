      ...
      if (isNew) {
        ...
        const newRuleset = await tx.tournamentRuleset.create({
          data: {
            ...
            thirdPlaceMatchEnabled: dto.thirdPlaceMatchEnabled ?? false,
            quickScoreEntryEnabled: dto.quickScoreEntryEnabled ?? false,
            requireLineup: dto.requireLineup ?? true,
            groupCount: dto.groupCount ?? 2,
            advancePerGroup: dto.advancePerGroup ?? 1,
            knockoutBracketSize: dto.knockoutBracketSize ?? null,
            knockoutSeedSlots: dto.knockoutSeedSlots ? (dto.knockoutSeedSlots as any) : [],
            isTemplate: false,
            createdById: userId,
          },
        });
